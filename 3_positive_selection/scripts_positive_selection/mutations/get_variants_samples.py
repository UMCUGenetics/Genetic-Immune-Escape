...
@click.option('--output_file',
              ...
              help="Output file",
              required=True)
def process_mutations(patients_file, ttype_name,output_file):
    ...
    for sample in paths_hmf:
        if not(os.path.exists(f"/hpc/cuppen/projects/P0020_genetics_immune_escape/large_scale_primary_met/processed/hmf/vaf/{sample}/{sample}.vaf_info.tsv.gz")): # this is the tab-delimited files created by the 0_process step
            continue
        df = pd.read_csv(
